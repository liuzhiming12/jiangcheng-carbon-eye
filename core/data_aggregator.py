...
def aggregate_emissions(data: pd.DataFrame, group_by: str = "project") -> pd.DataFrame:
    ...
    if group_by == "project":
        ...
        raise ValueError(
            f"Unsupported aggregation dimension: '{group_by}'. "
            f"Supported values: project, file_path, hour, day, week, month, quarter, year."
        )

    # Ensure numeric columns are float64 to avoid int64 overflow
    for col in ['emissions', 'energy_consumption', 'duration']:
